...
interface Props {
  ...
  onClose: () => void;
  data: Gudang | null;
  onUpdate: (id: number, data: { nama_gudang: string; tipe_gudang: string; alamat_gudang: string }) => void;
}

const ModalEditGudang: FC<Props> = ({ isOpen, onClose, data, onUpdate }) => {
  const [nama, setNama] = useState("");
  const [tipe, setTipe] = useState("");
  const [alamat, setAlamat] = useState("");

  // ⬇ isi form tiap kali data berubah
  useEffect(() => {
    if (data) {
      setNama(data.nama_gudang || "");
      setTipe(data.tipe_gudang || "");
      setAlamat(data.alamat_gudang || "");
    }
  }, [data]);

  // ⬇ wajib reset saat modal ditutup
  const handleClose = () => {
    setNama("");
    setTipe("");
    setAlamat("");
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-md">
        <h2 className="text-lg font-bold mb-4">Edit Gudang</h2>
...
export default ModalEditGudang;
